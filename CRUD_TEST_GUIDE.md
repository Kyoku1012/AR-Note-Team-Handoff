...
2. Click `Test Note`.
3. Expected result:
   - A debug note appears in front of the camera.
   - The edit panel opens.
   - Console includes:

```text
Created debug test note without Vuforia plane detection.
```

If `Test Note` does not open the panel, click `Edit Note`. With no selected note, it should create and open a debug test note.
...
3. Expected result:
   ...
   - Clicking `Edit Note` creates a new debug note only if no notes remain.
...
Successful debug note creation:
